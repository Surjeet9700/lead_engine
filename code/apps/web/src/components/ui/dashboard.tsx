...
function StatGroup({
	...
	return (
		<div
			...
			className={cn("@container/stats overflow-hidden rounded-lg border border-border/80 bg-card/50", className)}
			{...props}
		>
			<div
				className={cn(
					"grid grid-cols-2 @2xl/stats:grid-cols-4",
					"[&:nth-child(2n)]:border-l border-border/60 [&:nth-child(n+3)]:border-t",
					"@2xl/stats:[&>*]:border-t-0 @2xl/stats:[&>*]:border-l border-border/60 @2xl/stats:[&>*:first-child]:border-l-0",
				)}
			>
				{children}
			</div>
...
function ChartCard({
	...
	return (
		<div
			...
			className={cn("flex flex-col rounded-lg border border-border/80 bg-card/50", className)}
			{...props}
		>
			...
			{footer ? (
				<div className="border-t border-border/60 px-5 py-3 text-muted-foreground text-xs md:px-6">
					{footer}
				</div>
			) : null}
		</div>
...
function KpiCard({
	...
	return (
		<div
			data-slot="kpi-card"
			className={cn("flex flex-col gap-4 rounded-lg border border-border/80 bg-card/50 p-5 md:p-6", className)}
			{...props}
		>
			...
			{hasHeader ? (
				<div className="flex flex-wrap items-end justify-between gap-x-4 gap-y-1">
					<div className="flex flex-col gap-1">
						{title ? (
							<h2 className="font-medium text-base tracking-tight text-foreground">{title}</h2>
						) : null}
						{description ? (
