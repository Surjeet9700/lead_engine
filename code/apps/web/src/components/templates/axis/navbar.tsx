...
const Navbar = () => {
	...
	const links = [
		...
		{ name: "Live Engine", href: "#speed-preview" },
	...
	return (
		...
		>
			<motion.div
				className="flex flex-row items-center justify-between px-3 py-2 rounded-full border border-border backdrop-blur-lg"
				animate={{
			...
			>
				...
				<section className="hidden flex-row items-center gap-4 lg:flex">
					<div className="flex flex-row gap-8">
						{links.map((link) => (
							<Link
								href={link.href}
								key={link.name}
								className="flex flex-row items-center gap-1 font-medium transition-colors hover:text-muted-foreground text-sm"
							>
								{link.name}
							</Link>
						))}
					...
					<Button
						variant="ghost"
						className="font-medium hover:!bg-transparent cursor-pointer transition-colors hover:text-muted-foreground text-sm"
